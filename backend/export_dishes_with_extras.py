...
# API - Теперь читаем из БД, эти значения только для fallback
...
# ============================================================================
# ЗАГРУЗКА НАСТРОЕК ИЗ БД (SQLite)
# ============================================================================

def load_settings_from_db():
    """
    Загрузить настройки StoreHouse из SQLite БД
    ...
    if not Path(db_path).exists():
        logging.warning(f"⚠️  БД не найдена по пути {db_path}, используем значения по умолчанию")
    ...
    try:
        ...
        if settings:
            logging.info("📝 Настройки StoreHouse загружены из БД SQLite")
            logging.info(f"   URL: {sh_url}")
            logging.info(f"   User: {sh_user}")
        else:
            logging.warning("⚠️  Настройки не найдены в БД, используем значения по умолчанию")

        return sh_url, sh_user, sh_pass

    except Exception as e:
        logging.error(f"❌ Ошибка загрузки настроек из БД: {e}")
        logging.warning("⚠️  Используем значения по умолчанию")
        return SH_URL, SH_USER, SH_PASS
...
async def call_procedure(session: aiohttp.ClientSession, proc_name: str,
                        inputs: List[Dict], sh_url: str, sh_user: str, sh_pass: str,
    ...
    try:
        ...
        if retry < MAX_RETRIES:
            ...
            return await call_procedure(session, proc_name, inputs, sh_url, sh_user, sh_pass, retry + 1)
...
async def get_goods_tree(session: aiohttp.ClientSession, sh_url: str, sh_user: str, sh_pass: str) -> List[Dict]:
...
async def get_dish_details(session: aiohttp.ClientSession, rid: int, sh_url: str, sh_user: str, sh_pass: str) -> Dict:
...
async def get_dish_composition(session: aiohttp.ClientSession, rid: int, sh_url: str, sh_user: str, sh_pass: str) -> List[Dict]:
...
async def fetch_extra_dish(session: aiohttp.ClientSession, rid: int, sh_url: str, sh_user: str, sh_pass: str) -> Optional[Dict]:
    """
    ...
    try:
        ...
        details = await get_dish_details(session, rid, sh_url, sh_user, sh_pass)

        # Запрос 2: GoodsItemCompDetail (состав)
        composition = await get_dish_composition(session, rid, sh_url, sh_user, sh_pass)
...
async def process_dish(session: aiohttp.ClientSession, goods_item: Dict,
                      semaphore: asyncio.Semaphore, sh_url: str, sh_user: str, sh_pass: str) -> Optional[Dict]:
            ...
            if add_list_sauce:
                ...
                if extra_rids:
                    ...
                    for extra_rid in extra_rids:
                        extra_data = await fetch_extra_dish(session, extra_rid, sh_url, sh_user, sh_pass)
                        if extra_data:
...
async def main():
    ...
    sh_url, sh_user, sh_pass = load_settings_from_db()
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        # Получаем все товары
        all_goods = await get_goods_tree(session, sh_url, sh_user, sh_pass)
        ...
        # Создаём задачи
        tasks = [process_dish(session, item, semaphore, sh_url, sh_user, sh_pass) for item in filtered_goods]
